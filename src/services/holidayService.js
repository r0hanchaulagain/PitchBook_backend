...
const { holidayFileExists, readHolidayFile, writeHolidayFile } = require('./holidayFileUtil');
...
async function fetchHolidaysForBSYear(bsYear) {
  ...
  const holidays = [];
  if (response.data && response.data.data && Array.isArray(response.data.data.dates)) {
