...
const generateChallenge = () => {
  try {
    ...
    // Create HMAC hash
    const hmac = crypto.createHmac(altchaConfig.algorithm, altchaConfig.hmacKey);
...
const verifyChallenge = (challenge) => {
  try {
    ...
    const challengeObj = typeof challenge === 'string' ? JSON.parse(challenge) : challenge;

    // Verify required fields
    const requiredFields = ['algorithm', 'challenge', 'signature', 'salt', 'hash'];
    ...
    const isVerified = verifier.verify(
      '-----BEGIN PUBLIC KEY-----\n' +
      'MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE' + challengeObj.publicKey + '\n' +
      '-----END PUBLIC KEY-----',
