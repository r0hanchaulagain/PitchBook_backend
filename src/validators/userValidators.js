...
const validatePasswordComplexity = (value) => {
	...
	// Check minimum length (8-16 characters)
	...
	const weakPatterns = [
		...
		'abc123', 'password123', 'admin123', 'test123'
	];
	...
	if (/(.)\1{3,}/.test(value)) {
		throw new Error('Password cannot contain more than 3 consecutive repeated characters');
	}
...
exports.registerValidator = [
	...
	body("role").isIn(["admin", "user", "futsalOwner"]).withMessage("Invalid role selected"),
...
exports.verifyMFAValidator = [
	body("token").optional().isLength({ min: 6, max: 6 }).withMessage("TOTP token must be 6 digits"),
